...
export function AdminStats({ stats }: AdminStatsProps) {
  ...
  const getColorClasses = (color: string) => {
    // Utiliser les couleurs du thème admin pour un look cohérent
    const colors = {
      blue: {
        bg: 'bg-blue-500/10',
        icon: 'text-blue-400',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat, index) => {
          ...
          return (
            <div key={index} className={`${themeClasses.card} ${colors.bg} rounded-lg p-6 border ${themeClasses.border}`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm font-medium ${themeClasses.textSecondary}`}>{stat.title}</p>
                  ...
                  <p className={`text-xs ${themeClasses.textSecondary} mt-1`}>{stat.description}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`${themeClasses.card} rounded-lg shadow p-6`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-lg font-semibold ${themeClasses.text}`}>Activité Utilisateurs</h3>
            <Activity className={`h-5 w-5 ${themeClasses.textSecondary}`} />
          </div>

          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className={`text-sm ${themeClasses.textSecondary}`}>Actifs cette semaine</span>
              <span className={`text-sm font-medium ${themeClasses.text}`}>
                {stats.active_last_week} / {stats.active_users}
              </span>
            </div>
            <div className={`w-full ${themeClasses.border} rounded-full h-2 bg-opacity-20`}>
              <div
        ...
        <div className={`${themeClasses.card} rounded-lg shadow p-6`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-lg font-semibold ${themeClasses.text}`}>Métriques de Croissance</h3>
            <TrendingUp className={`h-5 w-5 ${themeClasses.textSecondary}`} />
          </div>

          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className={`text-sm ${themeClasses.textSecondary}`}>Nouveaux cette semaine</span>
              <span className="text-sm font-medium text-green-400">
                +{stats.new_users_week}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className={`text-sm ${themeClasses.textSecondary}`}>Taux d'activation</span>
              <span className={`text-sm font-medium ${themeClasses.text}`}>
                {activePercentage}%
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className={`text-sm ${themeClasses.textSecondary}`}>Ratio Admin/Utilisateurs</span>
      ...
      {/* Quick Insights */}
      <div className={`${themeClasses.card} rounded-lg shadow p-6`}>
        <h3 className={`text-lg font-semibold ${themeClasses.text} mb-4`}>Informations Rapides</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className={`text-xs ${themeClasses.textSecondary}`}>Total comptes</p>
              <p className={`text-sm font-medium ${themeClasses.text}`}>{stats.total_users}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className={`text-xs ${themeClasses.textSecondary}`}>Comptes actifs</p>
              <p className={`text-sm font-medium ${themeClasses.text}`}>{stats.active_users}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className={`text-xs ${themeClasses.textSecondary}`}>Administrateurs</p>
              <p className={`text-sm font-medium ${themeClasses.text}`}>{stats.admin_users}</p>
            </div>
          </div>
        </div>
      </div>
